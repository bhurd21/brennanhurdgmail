...
# sort -> the ROW_NUMBER() ORDER BY. A fixed allow-list; never interpolate raw
# user text. "relevant" = prominent first, "irrelevant" = obscure first.
_ORDER = {
    "relevant":   "career_games DESC, last_season DESC NULLS LAST, player_id",
    "irrelevant": "career_games ASC,  last_season ASC  NULLS LAST, player_id",
}
...
def _drop_floor(fields: dict, sort: str) -> dict:
    """In irrelevant mode, blank the rate-stat playing-time floor so the
    marginal players we'd normally filter are surfaced."""
    fields = dict(fields)
    if sort == "irrelevant" and "floor_clause" in fields:
        fields["floor_clause"] = ""
    return fields
...
def _build_side(cond: classifiers.Condition, prefix: str, sort: str) -> tuple[str, dict]:
    """Render one condition's SQL with its params namespaced (a_/b_)."""
    fields = _drop_floor(cond.fields, sort)
    sql = _fragment(cond.fragment).format(p=prefix, **fields)
    params = {f"{prefix}{k}": v for k, v in cond.params.items()}
    return sql, params


def _normal_ctes(cond_a, cond_b, sort: str) -> tuple[str, dict]:
    """Two independent condition fragments, INTERSECTed on playerID."""
    frag_a, pa = _build_side(cond_a, "a_", sort)
    frag_b, pb = _build_side(cond_b, "b_", sort)
    ctes = (
        f"cond_a AS (\n{frag_a}\n),\n"
        f"cond_b AS (\n{frag_b}\n),\n"
        'matched_ids AS (\n'
...
def build(question: str, limit: int = 100, sort: str = "relevant"):
    ...
    if cond_a.category == "team" and _is_season_stat(cond_b):
        ctes, params = _combined_ctes(cond_a, cond_b, sort)
    elif cond_b.category == "team" and _is_season_stat(cond_a):
        ctes, params = _combined_ctes(cond_b, cond_a, sort)
    else:
        ctes, params = _normal_ctes(cond_a, cond_b, sort)

    sql = _envelope().format(ctes=ctes, order=_ORDER[sort])
    params["limit"] = limit
    category = "_".join(sorted([cond_a.category, cond_b.category]))
    return sql, params, category


async def answer(question: str, limit: int = 100, sort: str = "relevant") -> dict:
    sql, params, category = build(question, limit, sort)
    if sql is None:
